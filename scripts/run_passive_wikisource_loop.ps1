    ...
    try {
        ...
        if ($approved -gt 0) {
            ...
            $refresh = Invoke-WithScratchProfileRefreshLock -RepoRoot $RepoRoot -Owner "passive-wikisource:$RunId:cycle-$cycle" -Body {
                ...
                Write-LoopEvent -Event 'profile_refresh_lock_releasing' -Data @{ cycle = $cycle }
                [pscustomobject]@{
            ...
            Write-LoopEvent -Event 'cycle_completed' -Data @{
                ...
                cumulative_retained_records = [int]$refresh.materialized.counts.retained
                cumulative_train_tokens = $refresh.train_tokens
                token_gate_passed = $refresh.token_gate_passed
            }
        } else {
