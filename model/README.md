...
```powershell
python corpus_cleaning.py --source data/rap_english_clean_categorized_with_families.parquet --dedupe both --min-quality 0.70
```
...
```powershell
python model/build_training_data.py --config model/configs/dataset_config.special_tokens.json
python model/train_local_cuda.py --config model/configs/local_cuda_config.example.json --train-path model/data_special_tokens/train.jsonl --validation-path model/data_special_tokens/validation.jsonl --add-structural-special-tokens --max-steps 50
```

When `data/cleaned/categorized_rap_corpus_train.txt` exists, `model/train_local_cuda.py` prefers it automatically and logs the corpus path, record count, estimated tokens, included tiers, and report path at startup. It also copies `data/cleaned/corpus_cleaning_summary.json` into the LoRA output directory as `corpus_cleaning_summary.json`.

To force an older JSONL dataset, pass explicit paths:

```powershell
python model/train_local_cuda.py --train-path model/data_full_verses/train.jsonl --validation-path model/data_full_verses/validation.jsonl
```
...
```powershell
python model/train_local_cuda.py --config model/configs/local_cuda_cleaned_chunks_fast.json --max-steps 1000 --timing-log-steps 25
```
...
```powershell
python model/train_local_cuda.py --timing-log-steps 5
```
...
python model/build_verse_sections.py --source data/rap_english_clean_categorized_with_families.parquet --output model/data/full_verse_sections.parquet
python model/build_training_data.py --config model/configs/dataset_config.full_verses.json
python model/train_local_cuda.py --config model/configs/local_cuda_config.example.json --train-path model/data_full_verses/train.jsonl --validation-path model/data_full_verses/validation.jsonl --output-dir model/artifacts/qwen2.5-7b-rap-lora-full-verses --add-structural-special-tokens --max-steps 50
```

Local defaults live here:

```text
model/configs/local_cuda_config.example.json
```
...
```powershell
python model/generate_local.py --adapter-dir model/artifacts/qwen2.5-7b-rap-lora-local --title "Night Shift" --theme "working late, loneliness, ambition, city lights" --keywords "night, shift, city, lights, work, ambition"
```
...
```powershell
python model/run_fixed_generation_eval.py --adapter-dir model/artifacts/qwen2.5-7b-rap-lora-cleaned-chunks-768-ga4-1000 --output-md reports/generation_eval_cleaned_1000.md --output-jsonl reports/generation_eval_cleaned_1000.jsonl
```
