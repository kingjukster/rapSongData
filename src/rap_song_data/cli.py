#!/usr/bin/env python3
"""Primary rap corpus and training pipeline.
...
def _build_mutation_dataset(rows: List[dict], out_path: Path) -> int:
    ...
    for (song_id, section_id), bars in grouped.items():
        ...
        for i in range(len(bars) - 2):
            ...
            if len(output_bars) < 2 or len(output_bars) != target_count:
                continue
            first = input_bars[0]
            out.append({
                ...
                "metadata": {
                    ...
                    "split": first["split"],
                    "input_bar_count": len(input_bars),
...
def _row_word_count(row: dict) -> int:
    ...
    try:
        ...
        pass
    return len(re.findall(r"[A-Za-z0-9']+", str(row.get("clean_bar_text") or row.get("bar_text") or "")))
...
def _build_preference_dataset(
    ...
    for bars in grouped.values():
        ...
        if quality_chosen <= quality_rejected:
            continue
        if quality_chosen - quality_rejected < min_quality_delta:
            continue
        prompt = _generate_sections_prompt(bars, bars[0]["section_id"])
        preferred = high["clean_bar_text"]
        rejected = low["clean_bar_text"]
        if _normalize_preference_text(preferred) == _normalize_preference_text(rejected):
            continue
        out.append({
            "prompt": prompt,
            "chosen": preferred,
            ...
            "metadata": {
                ...
                "quality_delta": round(quality_chosen - quality_rejected, 4),
            },
        })
...
def cmd_build_datasets(args: argparse.Namespace):
    ...
    license_filtered_rows = len(rows)
    rows = [r for r in rows if float(r.get("quality_score", 0.0) or 0.0) >= float(args.min_quality_score)]
    quality_filtered_rows = len(rows)
    short_allowed_sections = {"intro", "adlib", "spoken"}
    rows = [
        r
        for r in rows
        if _row_word_count(r) >= int(args.min_word_count)
        or str(r.get("section_type", "")).lower() in short_allowed_sections
    ]
    length_filtered_rows = len(rows)
    if quality_filtered_rows == 0:
        raise ValueError(
            "No rows remain after license and quality filtering. "
            "Use --include-risk only if you intentionally accept unknown-source rows."
        )
    if length_filtered_rows == 0:
        raise ValueError("No rows remain after min word count filtering.")
    ...
    preference_count = _build_preference_dataset(
        rows,
        Path(args.preference_out),
        min_quality_delta=float(args.preference_min_delta),
    )

    summary = {
        "input_rows": input_rows,
        "license_filtered_rows": license_filtered_rows,
        "quality_filtered_rows": quality_filtered_rows,
        "length_filtered_rows": length_filtered_rows,
        "min_quality_score": float(args.min_quality_score),
        "min_word_count": int(args.min_word_count),
        "preference_min_delta": float(args.preference_min_delta),
...
def _load_config(path: Optional[str], args: argparse.Namespace) -> argparse.Namespace:
    ...
    section_name = {"build-datasets": "datasets"}.get(args.command, args.command)
    values = {key: value for key, value in cfg.items() if not isinstance(value, dict)}
    ...
    supplied_flags = {
        token.split("=", 1)[0]
        for token in os.sys.argv[1:]
        if token.startswith("--")
    }
    for key, value in values.items():
        if not hasattr(args, key):
            continue
        flag_names = {f"--{key}", f"--{key.replace('_', '-') }"}
        if supplied_flags.isdisjoint(flag_names):
...
def main():
    ...
    args.min_word_count = getattr(args, "min_word_count", 5)
    args.min_quality_score = getattr(args, "min_quality_score", 0.70)
...
def cmd_train(args: argparse.Namespace):
    ...
    train_path = Path(train_target)
    is_module = not train_target.endswith(".py") and not train_path.exists()
    command = [os.sys.executable, "-m", train_target] if is_module else [os.sys.executable, train_target]
    is_local_trainer = (
        train_target == "rap_song_data.training.local_cuda"
        or train_path.stem.lower() in {"train_local_cuda", "local_cuda"}
    )
    if is_local_trainer:
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p_curate.add_argument("--min-quality-score", type=float, default=0.70)
    p_curate.add_argument("--min-word-count", type=int, default=5)
    ...
    p_dataset.add_argument("--run-dir", default="runs/dataset-builds")
    ...
    p_dataset.add_argument("--min-quality-score", type=float, default=0.70)
    p_dataset.add_argument("--min-word-count", type=int, default=5)
    p_dataset.add_argument("--preference-min-delta", type=float, default=0.25)
    ...
    p_train.add_argument("--train-script", default="rap_song_data.training.local_cuda")
    ...
    p_train.add_argument("--run-dir", default="runs/training")
    ...
    p_gen.add_argument("--run-dir", default="runs/generation")
    ...
    p_audit.add_argument("--out", default="reports/label_audit.md")
