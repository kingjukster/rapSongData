...
def cmd_train(args: argparse.Namespace):
    ...
    if train_script.name.lower() == "train_local_cuda.py" and not train_script.exists():
        default_local = Path("model") / "train_local_cuda.py"
        if default_local.exists():
            train_script = default_local
    comparability_note = getattr(args, "comparability_note", "default") or ("smoke" if args.smoke else "default")
    ...
    if train_script.name.lower().startswith("train_local_cuda"):
        ...
        command += ["--load-in-4bit"] if getattr(args, "load_in_4bit", True) else ["--no-load-in-4bit"]
        if getattr(args, "skip_smoke_check", False):
            command += ["--skip-smoke-check"]
        command += ["--comparability_note", comparability_note]
    else:
        command = [os.sys.executable, str(train_script)]
